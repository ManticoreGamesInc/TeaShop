...
function set_animation_state(state)
	...
	last_state = state
	script:SetNetworkedCustomProperty("state", state)
end
...
function activities.safe.tick(activity, dt)
...
function activities.safe.tickHighestPriority(activity, dt)
...
function activities.scared.start(activity)
	set_animation_state(1)
	move_ai(hide_target, 2, run_speed)
end

function activities.scared.tick(activity)
	activity.priority = 100
end

function Tick()
	in_range = #Game.FindPlayersInSphere(root:GetWorldPosition(), 700)

	if(in_range > 0) then
		last_in_range_time = time()
	end

	CoreDebug.DrawSphere(root:GetWorldPosition(), 700, {
		duration = 0.01,
		color = Color.RED
	})
end
